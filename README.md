...
En la carpeta `Modelo` está contenido el archivo `MiBiciTuBici.alp` que contiene el modelo listo para ser ejecutado en el [software de simulación Anylogic](anylogic.com). Dentro del mismo directorio se encuentra el script `query_system_state.py` que actualiza la base de datos del modelo con las condiciones actuales para poder así ejecutar la simulación con el mínimo estado transitorio posible.
